...
const Carousel: Taro.FC<CarouselProps> = props => {
  ...
  return (
    <Swiper
      style={{ backgroundColor: 'rgba(0, 0, 0, 0.1)', overflow: 'hidden', ...style }}
    ...
    >
      {list.map((item, index) => {
        ...
        // 默认自动播放
        const isVideoAutoplay = item.autoplay === undefined ? true : item.autoplay;
        return (
          <SwiperItem key={src} onClick={url ? () => Taro.navigateTo({ url }) : () => {}}>
            {type === 'image' && (
              <Image mode="aspectFill" style={{ width: '100%', height: '100%' }} src={src} />
            )}
            {type === 'video' && (
              <Video
